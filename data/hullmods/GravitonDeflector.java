...
public class GravitonDeflector extends BaseHullMod {
    ...
    static final float FORCE_MULTIPLIER = 0.7f;
    ...
    public void advanceInCombat(ShipAPI ship, float amount) {
        ...
        //for(Iterator iter = CombatUtils.getProjectilesWithinRange(ship.getLocation(), ship.getCollisionRadius() * 6).iterator(); iter.hasNext();) {
        for(Iterator iter = Global.getCombatEngine().getProjectiles().iterator(); iter.hasNext();) {
            ...
            float force = (float)Math.pow(1 - Math.abs(angleDif) / 30, 2)

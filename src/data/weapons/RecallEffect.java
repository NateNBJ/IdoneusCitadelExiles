...
//            weapon.setAmmo(ammo);
    ...
    void teleport(ShipAPI ship, Vector2f to) {
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        if (onFireEffectIsReady && weapon.getCooldownRemaining() > 0) {
            ...
            for(ShipAPI ally : AIUtils.getAlliesOnMap(ship)) {
                AssignmentInfo asgnmt = engine.getFleetManager(ship.getOwner()).getAssignmentFor(ally);
                
                if(asgnmt == null || asgnmt.getTarget() == null
                        || MathUtils.getDistance(ally, asgnmt.getTarget().getLocation())
                            < MathUtils.getDistance(ship, asgnmt.getTarget().getLocation()))
                    continue;
                
                SunUtils.print(ally, "RECALL");    
            }
